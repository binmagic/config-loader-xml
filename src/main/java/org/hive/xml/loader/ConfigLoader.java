package org.hive.xml.loader;

import org.hive.xml.ConfigFactory;
import org.hive.xml.ConfigListener;
import org.hive.xml.SpecialAppendPath;
import org.hive.xml.exception.ConfigException;
import org.hive.xml.exception.ConfigExceptionHandler;
import org.hive.xml.format.DateFormatTransformer;
import org.hive.xml.format.TimeSpanTransformer;
import org.hive.xml.util.StringUtil;
...
{
	private static ConfigLoader CONFIG_LOADER = new ConfigLoader(false);
	...
	{
		loadAll(factory, path, true, new ConfigExceptionHandler());
	}

	public static void loadAll(ConfigFactory factory, String path, boolean watch) throws ConfigException
	{
		loadAll(factory, path, watch, new ConfigExceptionHandler());
	}
	...
	private ConfigLoader(Boolean watch)
	{
		if(watch)
		{
	...
	public void reload() throws ConfigException
	{
		...
		//Settings.getInstance().loadJson(getPath() + File.separator + "version.json");
	...
	private <T> T load(Class<? extends T> type, Matcher matcher) throws ConfigException
	{
		...
		if(SpecialAppendPath.class.isAssignableFrom(type))
		{
			try
			{
				SpecialAppendPath instance = (SpecialAppendPath)type.newInstance();
				filePrefix = getPath() + File.separator + instance.specialAppendPath();
			}
		...
		if(StringUtil.isNullOrEmpty(filePrefix) && root != null && !StringUtil.isNullOrEmpty(root.name()))
		{
			filePrefix = getPath() + File.separator + root.name();
		}

		if(StringUtil.isNullOrEmpty(filePrefix))
		{
			filePrefix = getPath() + File.separator + type.getSimpleName().toLowerCase();
		}
		...
		else
		{
			...
			if(is.exists() && is.isDirectory())
			{
				try
				{
					...
					File[] files = is.listFiles(file1 -> file1.isFile() && file1.getName().endsWith(".xml"));
					if(files != null)
					{
						for(File file : files)
						{
